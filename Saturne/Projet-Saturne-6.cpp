// Création du projet de la planète Saturne : assemblage du disque et des deux demi-anneaux, couleur unie
...
int main()
{
    ...
    string nomFichierImage = chemin + "\\Saturne6.bmp";
    ...
    for (int i = 0; i <= x; ++i)
    {
        for (int j = 0; j <= y; ++j)
        {
            ...
            // l'anneau elliptique avant, mettre la couleur à rouge
            ...
            // du disque de Saturne, mettre la couleur à orange
            else if (estDansDemiAnneauElliptiqueArriere(j, i, yCentreSaturne, xCentreSaturne, GrandRayonInt, PetitRayonInt, GrandRayonExt, PetitRayonExt))
            {
                // Obtenez la couleur de l'anneau
                couleur = Orange;
            }
